...
const FacultyFormCreate = () => {
  ...
  const fetchForm = async () => {
    try {
      setLoading(true);
      const rawToken = localStorage.getItem('facultyToken');
  ...
  const handleFieldAdd = (fieldType) => {
    const newField = {
      ...
      placeholder: '',
      required: false,
      options: ['select', 'radio', 'checkbox'].includes(fieldType) ? [{ label: 'Option 1', value: 'option1' }] : [],
      validation: {},
      properties: {},
      styling: {
        width: '100%',
  ...
  if (loading) {
    return (
      <FacultyLayout title={isEdit ? 'Edit Form' : 'Create New Form'}>
      ...
      <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
        ...
        <Box sx={{ mb: 4 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
            ...
            <Button variant="contained" startIcon={<PublishIcon />} onClick={() => handleSave(true)} disabled={saving || formData.fields.length === 0}>
            ...
            <CardContent>
              <Grid container spacing={3}>
                <Grid item xs={12} md={6}>
                  <TextField fullWidth label="Form Title" value={formData.title} onChange={(e) => handleFormDataUpdate({ title: e.target.value })} placeholder="Enter form title" />
                </Grid>
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth>
                    <InputLabel>Visibility</InputLabel>
                    <Select value={formData.visibility} label="Visibility" onChange={(e) => handleFormDataUpdate({ visibility: e.target.value })}>
                ...
                <Grid item xs={12}>
                  <TextField fullWidth multiline rows={3} label="Description" value={formData.description} onChange={(e) => handleFormDataUpdate({ description: e.target.value })} placeholder="Enter form description (optional)" />
                </Grid>
              </Grid>
            </CardContent>
          </Card>
        ...
        <Card>
          ...
          <CardContent sx={{ minHeight: 600 }}>
            ...
            {activeTab === 1 && (
              <FormSettings settings={formData.settings} onSettingsUpdate={(settings) => handleFormDataUpdate({ settings })} />
            )}
            {activeTab === 2 && (
              <FormStyling styling={formData.styling} onStylingUpdate={(styling) => handleFormDataUpdate({ styling })} />
            )}
            {activeTab === 3 && (
              <FormPreview formData={formData} onClose={() => setActiveTab(0)} />
            )}
          </CardContent>
        </Card>
...
export default FacultyFormCreate;
