...
const FacultyFormBuilder = () => {
  ...
  const fetchForms = async () => {
    try {
      setLoading(true);
      const raw = localStorage.getItem('facultyToken');
  ...
  const handleEdit = (form) => {
    navigate(`/faculty/forms/edit/${form._id}`);
  };

  const handleView = (form) => {
    navigate(`/faculty/forms/view/${form._id}`);
  };

  const handleAnalytics = (form) => {
    navigate(`/faculty/forms/analytics/${form._id}`);
  };
  ...
  if (loading) {
    return (
      <FacultyLayout title="Form Builder">
      ...
      <Box sx={{ mb: 4 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          ...
          <Button
            variant="contained"
            startIcon={<AddIcon />}
            onClick={() => navigate('/faculty/forms/create')}
            sx={{ borderRadius: 2 }}
          >
            Create New Form
          </Button>
...
export default FacultyFormBuilder;
