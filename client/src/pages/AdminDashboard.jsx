...
const AdminDashboard = () => {
  ...
  const handleAssignAllStudentsToHostels = async () => {
    try {
      setIsAssigningStudents(true);
      const response = await api.post('/api/admin/assign-all-students-to-hostels');
      
      if (response.data.success) {
        toast.success(response.data.message);
        // Refresh statistics after assignment
        await fetchStatistics();
      } else {
        toast.error(response.data.message || 'Failed to assign students to hostels');
      }
    } catch (error) {
      console.error('Error assigning students to hostels:', error);
      toast.error(error.response?.data?.message || 'Failed to assign students to hostels');
    } finally {
