...
// Add new department
export const adminAddDepartment = (departmentData) => {
  return async (dispatch) => {
    try {
      console.log('API call starting with data:', departmentData);
      const { data } = await api.post("/api/admin/addDepartment", departmentData);
      ...
      return { 
        success: false, 
        error: err.response?.data || { message: "Failed to add department" } 
      };
    }
  };
};
...
// Get all departments
export const adminGetAllDepartments = (search = "", isActive = null) => {
  return async (dispatch) => {
    try {
      ...
      console.log('Fetching departments with params:', { search, isActive });
      const { data } = await api.get(`/api/admin/departments?${params.toString()}`);
...
export const adminGetDepartmentById = (id) => {
  return async (dispatch) => {
    try {
      const { data } = await api.get(`/api/admin/department/${id}`);
      return { success: true, data: data.result };
    } catch (err) {
      return { 
        success: false, 
...
export const adminUpdateDepartment = (id, departmentData) => {
  return async (dispatch) => {
    try {
      const { data } = await api.put(`/api/admin/department/${id}`, departmentData);
      return { success: true, data: data.result };
    } catch (err) {
      return { 
        success: false, 
...
export const adminDeleteDepartment = (id) => {
  return async (dispatch) => {
    try {
      ...
      const { data } = await api.delete(`/api/admin/department/${id}`);
...
export const adminToggleDepartmentStatus = (id, isActive) => {
  return async (dispatch) => {
    try {
      console.log('Toggling department status:', { id, isActive });
      const { data } = await api.patch(`/api/admin/department/${id}/status`, { isActive });
      ...
      return { 
        success: false, 
        error: err.response?.data || { message: "Failed to update department status" } 
      };
    }
  };
};
